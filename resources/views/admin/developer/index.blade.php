    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-header-title">
                <h4 class="pull-left page-title">List Of developer</h4>
        ...
        <div class="col-md-12">
            <div class="panel panel-primary">
                <div class="panel-heading">
                    <h3 class="panel-title">developer List</h3>
                </div>
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered text-center">
                        ...
                        @foreach($developers as $developer)
                            <tr>
                                ...
                                <td>{{$developer->digination}}</td>
                                <td>{{$developer->location}}</td>
                                <td>{{$developer->degree}}</td>
                                <td>{{$developer->complete}}</td>
                                <td>{{$developer->email}}</td>
                                <td>{{$developer->phone}}</td>
                                <td>{{Str::limit($developer->facebook,10)}}...</td>
                                <td>{{Str::limit($developer->twitter,10)}}...</td>
                                <td>{{Str::limit($developer->linkedin,10)}}...</td>
                                <td>{{Str::limit($developer->description,50)}}...</td>
                                <td class="text-center">
                                    <div  style="max-width: 70px; max-height:70px;overflow:hidden">
                                        <img src="{{ asset($developer->image) }}" class="img-fluid img-rounded" alt="">
                                    </div>
                                </td>
                                <td class="d-flex">
                                    <a class="btn btn-info d-inline-block" href="{{ route('developer.edit', $developer->id) }}"><i class="fa fa-edit"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>

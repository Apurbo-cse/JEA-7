    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-header-title">
                <h4 class="pull-left page-title">Mission/Vission/History</h4>
                <ol class="breadcrumb pull-right">
                    <li><a href="{{route('dashboard')}}">Dashboard</a></li>
                    <li><a href="{{route('aboutm.create')}}">Create New</a></li>
                    <li class="active">Detailes</li>
                </ol>
        ...
        <div class="col-md-12">
            <div class="panel panel-primary">
                <div class="panel-heading">
                    <h3 class="panel-title">M-V-H List</h3>
                </div>
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered text-center">
                        ...
                        @foreach($aboutms as $aboutm)
                            <tr>
                                <td>{{$serial++ }}</td>
                                <td>{{$aboutm->title}}</td>
                                <td>{{Str::limit($aboutm->description, 100)}}...</td>
                                <td>{{ucfirst($aboutm->status)}}</td>
                                <td class="text-center">
                                    <div style="width:110px; height:90px;overflow:hidden">
                                        <img src="{{ asset($aboutm->image) }}" style="width:100px; height:80px;overflow:hidden" alt="">
                                    </div>

                                </td>
                                <td class="d-flex">
                                    <a class="btn btn-info d-inline-block" href="{{ route('aboutm.edit', $aboutm->id) }}">Edit</a>
                                    <form class="d-inline-block pull-right" method="post" action="{{ route('aboutm.destroy', $aboutm->id) }}">
                                        @csrf
                                        @method('delete')
